...
var topology_config = {
    ...
    link_factor: 150, // factor for link length (if link.length > 0)
...
function Topology(conf) {
    ...
    function update(data) {
        ...
        //console.log(data)
        // Update link class to show attacks.
        var links = svg.selectAll('#topology .link');
        links.style().classed('linkattack', function(link) {
            var val = false
            data.a_branches[0].forEach(function(hlink) {
                if (link.branch == hlink) {
                    val = true
                }
            })
            return val
        })
        links.style().classed('linkfix', function(link) {
            var val = false
            data.f_branches.forEach(function(hlink) {
                if (link.branch == hlink) {
                    val = true
                }
            })
        ...
        // Update node class to show attacks.
        nodes.style().classed('nodeattack', function(node) {
            var val = false
            data.a_nodes[0].forEach(function(hnode) {
                if (node.name.includes(hnode)) {
                    val = true
                }
            })
            return val
        })
        nodes.style().classed('nodefix', function(node) {
            var val = false
            data.f_nodes.forEach(function(hnode) {
                if (node.name.includes(hnode)) {
                    val = true
                }
            })
