...
class PyPower(mosaik_api.Simulator):
    ...
    def create(self, num, modelname, gridfile, sheetnames=None):
        ...
        for i in range(num):
            ...
            ppc, entities = model.load_case(
                self.gridfile, grid_idx, sheetnames)
    ...
    def step(self, time, inputs):
        ...
        for eid, attrs in inputs.items():
            if eid == self.grideid:
                # reload the file with new grid. // TODO: see if there is an
                # easier way for it
                grids = []
                self._entities = {}
                self._ppcs = []
                for name, values in attrs.items():
                    if name == 'newgrid':
                        self.newgrid = values['TopologySim-0.Topology']
                        grid_idx = 0
                        sheetnames = {}
                        ppc, entities = model.load_case(
                            self.newgrid, grid_idx, sheetnames)
                        self._ppcs.append(ppc)
            ...
            else:  # not sure if the rest here should be indeed indented.
    ...
    """
    Returns a dict containing the values of each attribute mapped to each part of the grid
    """
    ...
    def get_data(self, outputs):
        ...
        for eid, attrs in outputs.items():
            for attr in attrs:
                try:
                    if eid == self.grideid and attr == 'newgrid':
                        val = self.newgrid
                    else:
